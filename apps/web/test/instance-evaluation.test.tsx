...
/**
 * Mention de version d'évaluation — CLAUDE.md §9.18 et §9.21.
 *
 ...
 */
describe('mention de version d’évaluation', () => {
  it('annonce l’évaluation quand l’instance le déclare', async () => {
    simulerApi({ '/api/instance': () => reponse(200, { evaluation: true }) });
    afficher(<LoginPage />, null);

    const bandeau = await screen.findByTestId('bandeau-evaluation');
    expect(bandeau.textContent).toMatch(/aucune conversation réelle/i);
  });

  it('ne dit rien sur une instance ordinaire', async () => {
    simulerApi({ '/api/instance': () => reponse(200, { evaluation: false }) });
    afficher(<LoginPage />, null);

    await screen.findByRole('button', { name: 'Se connecter' });
    await waitFor(() => {
      expect(screen.queryByTestId('bandeau-evaluation')).toBeNull();
    });
  });
